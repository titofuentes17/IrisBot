...
#command_list = ['kda', 'winrate', 'masteries', 'all_masteries', 'pings', 'summoner', 'ingame']
...
def all_mastery_func (riot_id='titofuentes#euw',n = 20):
    print('inicio mastery all')
    masterii = []
    riot_id = riot_id.replace(' ', '%20')
    riot_id = riot_id.replace('#', '/')
    rq = requests.get(f'https://europe.api.riotgames.com/riot/account/v1/accounts/by-riot-id/{riot_id}?api_key={api_key}').json()
...
@bot.command()
async def all_masteries(ctx,*riot_id:str):
    good_id = ''.join(riot_id)
    resultado = all_mastery_func(good_id)
