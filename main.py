...
def wr(riot_id='titofuentes#euw',n = 20):
    print('inicio wr')
    win = 0
    riot_id = riot_id.replace(' ', '%20')
    riot_id = riot_id.replace('#', '/')
    ...
    for match in match_list:
        ...
        if match_data['info']['participants'][i]['win']:
            win = win +1
    print('fin wr')
    return f' {rq["gameName"]} WR: {(win/n)*100} % ({win} out of {n})'

def pings(riot_id='titofuentes#euw',n = 20):
    print('inicio pings')
...
@bot.command()
async def kda(ctx,riot_id):
    resultado = kda_func(riot_id)
